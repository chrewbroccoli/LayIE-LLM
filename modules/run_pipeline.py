...
import shutil
import tempfile
from modules.data_processing import load_dataset, categorize_files_by_template, extract_text_from_pdf, process_ground_truth_annotations
                        ...
                        try:
                            ...
                            with tempfile.TemporaryDirectory() as temp_dir:
                                ...
                                if model == MODEL_llama:
                                    ...
                                    ground_truth = process_ground_truth_annotations(filename, ground_truth_annotations)
...
def run_experiment(directory, base_output_directory, dataset_path, dtype, folder_path,  model, params, output_file=None):
    ...
    if model == MODEL_llama:
        ...
        try:
            filename_mapping, output_data = perform_experiment(
                 directory, base_output_directory, dataset_path, dtype, folder_path,  model, sample_size, use_custom_ocr, experiment_id, prompt_type, chunk_size_category, output_file, **kwargs
            )
            if output_data:
